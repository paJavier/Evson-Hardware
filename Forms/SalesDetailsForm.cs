        ...
        private void LoadSaleDetails(int saleId)
        {
            try
            {
                ...
                using (var r = hCmd.ExecuteReader())
                {
                    if (r.Read())
                    {
                        lblReceiptVal.Text = r["receipt_number"]?.ToString() ?? "—";
                        lblDateVal.Text = r["sale_date"]?.ToString() ?? "—";
                        lblCustomerVal.Text = r["customer_name"]?.ToString() ?? "Walk-in";
                        lblTotalVal.Text = Convert.ToDecimal(r["total_amount"])
                                               .ToString("C2", PhCulture);
                    }
                }
